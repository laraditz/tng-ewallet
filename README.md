...
| `TNG_ENCRYPTION_KEY` | Encrypts `access_token`/`refresh_token` at rest and keys the `access_token_hash` lookup — dedicated to this package, independent of `APP_KEY`. Generate one with `php artisan tng-ewallet:generate-key` | *(required if using the Agreement Payment / access-token flow)* |

`client_id`, `partner_id`, and `private_key_path` are required — a missing value throws `ConfigurationException` before any HTTP call is made. `encryption_key` is required only once an `AccessToken` row is actually read or written (i.e. the Agreement Payment flow) — a Cashier-Payment-only integration never needs to set it.
...
- `access_token` and `refresh_token` (in `tng_ewallet_access_tokens`) are both encrypted at rest, using `TNG_ENCRYPTION_KEY` — a key dedicated to this package, deliberately independent of your app's `APP_KEY`. Since encryption here is non-deterministic (a random IV per encryption), `cancelToken()`/`user()` can't look up a row with `WHERE access_token = ?` against the encrypted column — a separate `access_token_hash` column (a deterministic HMAC-SHA256, keyed on the same `TNG_ENCRYPTION_KEY`) is used for that lookup instead, while the actual token value stays encrypted. Apply database-level access restrictions to this table regardless — it holds bearer credentials that authorize auto-debit charges. **`TNG_ENCRYPTION_KEY` is designed to never rotate** — access tokens already have their own TNG-driven expiry/rotation lifecycle, so there's no need to rotate this key the way you'd rotate `APP_KEY`. There is no built-in migration path if you change it: existing encrypted rows become permanently undecryptable, and existing hash lookups stop matching. Treat it as a fixed secret, generated once via `php artisan tng-ewallet:generate-key` and never touched again — including by any blanket "rotate all secrets" tooling or policy, which should explicitly exclude it.
- `tng_ewallet_api_logs.request_payload`/`response_payload` are stored as plain JSON, not encrypted — but `accessToken`, `refreshToken`, and `authCode` are redacted (replaced with a `[redacted:{hash}]` marker, using the same HMAC as `access_token_hash`) before a row is ever written, since those are the only fields in a logged payload that are actually sensitive. A redacted marker stays correlatable to its `tng_ewallet_access_tokens` row via `access_token_hash` without ever storing the recoverable secret. The rest of a logged payload (order titles, amounts, customer IDs) is ordinary transactional data.
